...
if force_tv and force_movie:
    print(
        "[WARNING] *** Force TV and Force Movie have been set. Check for duplicate categories. ***"
    )
    sys.exit(POSTPROCESS_ERROR)
                    ...
                    if verbose:
                        if subpart != "":
                            print(
                                ...
                                % (filename, guess["subtitle_language"][0])
                            )
                        else:
...
def add_series_mapping(guess, mapping):
    ...
    if not isinstance(guess.get("episode"), list):
        ...
        if multiple_episodes == "range":
            ...
            episode_num_just = (
                episodes[0].rjust(2, "0") + episode_separator + episodes[-1].rjust(2, "0")
            )
...
def deobfuscate_path(filename):
    ...
    if fn.find(".") == -1 and fn.find("_") == -1 and fn.find(" ") == -1:
        print(
            "Detected obfuscated filename %s, removing from guess path" % os.path.basename(filename)
        )
...
def guess_info(filename):
    ...
    guess = guessit.api.guessit(
        unicode(guessfilename),
        {"allowed_languages": [], "allowed_countries": [], "type": guessit_type},
    )
    ...
    if guess["type"] == "episode" and (guess.get("season") is None):
        guess["season"] = 1
        if verbose:
